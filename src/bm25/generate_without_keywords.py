...
def main():
    ...
    for id, query in enumerate(tqdm(queries)):
        for mode in ["naive", "hyde", "rewrite"]:
            ...
            file_path = f"/home/stv10121/RUS/bm25_result/keyword_none_{mode}.json"
            ...
            if mode == "naive":
                ...
                # hyde
                hop_1_query = get_virtual_abstract(query)
            else: # mode == "rewrite"
                # rewrite
                hop_1_query = get_rewrited_query(query)

            # 1-hop retrieval
            hop_1_paper = _retrieve(hop_1_query, doc_list, bm25, n=1)[0]
    
            # 2-hop retrieval
            hop_2_query = hop_1_paper + hop_1_query
            hop_2_papers = _retrieve(hop_2_query, doc_list, bm25, n=2)

            # replace duplicated documents
            hop_2_paper = hop_2_papers[1] if hop_2_papers[0] == hop_1_paper else hop_2_papers[1]

            papers = f"<p1>{hop_1_paper}</p1><p2>{hop_2_paper}</p2>"

            # generate answer by LLM
            output = generate(query, papers)
            ...
            result_dict["data"].append(
                {
                    ...
                    "results": [
                    {
                        f"query_{mode}": hop_1_query,
                        "1_hop_paper": hop_1_paper,
