...
def main(cfg: DictConfig):
    ...
    #########################################################################################
    # result 파일 선언
    json_file = "/home/stv10121/RUS/DPR/DPR_result/result.json"
    f = open(json_file, 'w', encoding='utf8')

    # 질문에 대해 n-hop retrieval
    for query_id in range(len(queries)):
        question = queries[query_id]
        expanded_query = questions[query_id]

        scores = []
        for p_id, paper in enumerate(tqdm(result_doc[query_id])):
            paper_text = paper[0]
            # retrieval 점수 측정
            scores.append((get_retriever_score(expanded_query, paper_text),p_id))

        scores = sorted(scores, key=lambda paper: paper[0], reverse=True)[:10]

        # document reranking
        hop_1_papers = [result_doc[query_id][score[1]][0] for score in scores]
        
        # hop_1_paper = get_merged_abstract(hop_1_papers)
        hop_1_paper = hop_1_papers[0]
        
        # Regeneration
        hop_1_summary = get_query_aspect_abstract_summary(question, hop_1_paper)

        # query refine
        refine_query = expanded_query + ' ' + hop_1_summary
        
        # 2-hop retrieval
        questions_tensor = retriever.generate_question_vectors([refine_query], query_token=qa_src.special_query_token)
        hop_2_paper_ids = retriever.get_top_docs(questions_tensor.numpy(), cfg.n_docs)[0][0]
        hop_2_paper = all_passages[hop_2_paper_ids[0]][0] if hop_2_paper_ids[0] != top_results_and_scores[query_id][0][0]  else  all_passages[hop_2_paper_ids[1]][0]        
        
        result_dict["data"].append({"question": question, "expanded_query":expanded_query,
                    "1_hop_paper": hop_1_paper, "2-hop_expanded_query": refine_query,
