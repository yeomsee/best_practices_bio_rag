...
def main(cfg: DictConfig):
    ...
    queries = [query.replace('\n', "") if '\n' in query else query for query in queries][:10]
    ...
    for exp in exp_list:
        result_dict = {"data": []}
        json_file = f"/home/stv10121/RUS/DPR_result/{exp}_keyword_mesh.json"
        ...
        for query in (queries):
            if exp == "exp_1": # naive
                ...
                hop_1_inputs = genereate_keywords(query)
                ...
                hop_1_inputs = get_virtual_abstract(query, aspect=None)
                hop_1_inputs = preprocess_text(hop_1_inputs)
            ...
            elif exp == "exp_4":  # keyword 생성 후 query+keyword -> keyword에 따른 질문 생성
                keywords = genereate_keywords(query)
