    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            if max_vector_len != min_vector_len:
                ...
                from DPR.dpr.models.reader import _pad_to_len
...
def main(cfg: DictConfig):
    from api_call import get_virtual_abstract, get_query_aspect_abstract_summary, get_merged_abstract, get_retriever_score
    from tqdm import tqdm
    ...
    for i in range(len(queries)):
        question = queries[i]
        questions.append(get_virtual_abstract(question))
    ...
    # result_doc = [[all_passages[e[0][k]] for k in range(100)] for e in top_results_and_scores]
    
    
    #########################################################################################

    for exp in ["exp_2", "exp_4"]:
        for mode in ["limit_3", "no_limit"]:
            result_dict = {"data": []}
            datas = load_file("DPR", exp, mode)
            ...
            if exp == "exp_2":
                for data in tqdm(datas):
                    ...
                    for result in results:
                        rewritten_queries.append(result["rewrite_query"])

                    # limit queries retrieve
                    rewritten_queries_tensor = retriever.generate_question_vectors(rewritten_queries,
                                                                            query_token=qa_src.special_query_token)
                    top_results_and_scores = retriever.get_top_docs(rewritten_queries_tensor.numpy(), cfg.n_docs)
                    all_passages = get_all_passages(ctx_sources)
                    hop_1_outputs = [[all_passages[e[0][k]][0] for k in range(5)] for e in top_results_and_scores]
                            ...
                            "results": [
                                {
                                    ...
                                    "1_hop_paper": hop_1_outputs[q_idx][0]
                                } for q_idx, rewrite_query in enumerate(rewritten_queries)
                            ]
                            })
            elif exp == "exp_4":
                for data in tqdm(datas):
            ...
            json_file = f"/home/stv10121/RUS/DPR_result/{exp}_1_hop_{mode}.json"
        
            with open(json_file, 'w', encoding='utf8') as out_file:
                json.dump(result_dict, out_file, indent="\t", ensure_ascii=False)

    # exp_list = ["exp_2", "exp_4"]
    # for exp in exp_list:
    #     result_dict = {"data": []}
    #     json_file = f"/home/stv10121/RUS/DPR_result/{exp}_limit_3.json"
    #     out_file = open(json_file, 'w', encoding='utf8')

    #     print(f"\n##### {exp} Data Preparation #####")
    ...
    #             print(f"\n##### {exp} Rewrite Diversity Check #####")
    #             rewrite_diversity_score = get_queries_diversity_score(query, hop_1_inputs)
    #             # if "rewrite" not in diversity_scores.keys():
    #             #     diversity_scores["rewrite"] = [rewrite_diversity_score]
    #             # else:
    #             #     diversity_scores["rewrite"].extend([rewrite_diversity_score])

    #             print(f"\n##### {exp} Rewrite Fitness Check #####")
    #             rewrite_fitness_score = get_fitness_score(query, hop_1_inputs)
    #             # fitness_scores.append(rewrite_fitness_score)

    #         elif exp == "exp_3":  # HyDE 방식에 따른 query expansion
    ...
    #             # print(f"\n##### {exp} Aspects Diversity Check #####")
    #             # aspect_diversity_score = get_aspects_diversity_score(query, aspects)
    #             # if "aspects" not in diversity_scores.keys():
    #             #     diversity_scores["aspects"] = [aspect_diversity_score]
    #             # else:
    #             #     diversity_scores["aspects"].extend([aspect_diversity_score])
    ...
    #         print(f"\n##### {exp} 1-hop Retrieval #####")
    #         hop_1_inputs_tensor = retriever.generate_question_vectors(hop_1_inputs,
    #                                                                   query_token=qa_src.special_query_token)
    #         top_results_and_scores = retriever.get_top_docs(hop_1_inputs_tensor.numpy(), cfg.n_docs)
    #         all_passages = get_all_passages(ctx_sources)
    ...
    #                  "results": [
    #                      {
    #                          "rewrite_query": hop_1_input,
    #                          "1_hop_paper": hop_1_outputs[q_idx][0]
    #                      } for q_idx, hop_1_input in enumerate(hop_1_inputs)
    #                  ],
    #                 "rewrite_diversity_score": rewrite_diversity_score,
    #                 "rewrite_fitness_score": rewrite_fitness_score,
    #                  })
    #         elif (exp == "exp_3"):
    #             result_dict["data"].append(
    #                 {"query_org": query,
