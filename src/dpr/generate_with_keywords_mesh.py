...
def main(cfg: DictConfig):
    ...
    for mode in ["hyde", "rewrite"]:
        in_file_path = f"/home/stv10121/RUS/bm25_result/keyword_mesh_{mode}.json"
        with open(in_file_path, 'r', encoding='utf-8') as in_file:
            datas = json.load(in_file)["data"]

        result_dict = {"data": []}
        print(f"##### keyword_mesh_{mode}_generation #####")
        for id, data in enumerate(tqdm(datas)):
            ...
            hop_1_papers, hop_2_papers = [], [] 
            for q_idx in range(len(keywords)):
                # 1-hop retrieval
                hop_1_query_tensor = retriever.generate_question_vectors([hop_1_queries[q_idx]],
                                                                        query_token=qa_src.special_query_token)
                top_results_and_scores = retriever.get_top_docs(hop_1_query_tensor.numpy(), cfg.n_docs)
                all_passages = get_all_passages(ctx_sources)
                hop_1_papers_temp = [[all_passages[e[0][k]][0] for k in range(5)] for e in top_results_and_scores]
                hop_1_paper = hop_1_papers_temp[0][0]
                hop_1_papers.append(hop_1_paper)

                # 2-hop retrieval
                hop_2_query = f"<p>{hop_1_paper}</p>{hop_1_queries[q_idx]}"
                hop_2_query_tensor = retriever.generate_question_vectors([hop_2_query],
                                                                        query_token=qa_src.special_query_token)
                top_results_and_scores = retriever.get_top_docs(hop_2_query_tensor.numpy(), cfg.n_docs)
                all_passages = get_all_passages(ctx_sources)
                hop_2_papers_temp = [[all_passages[e[0][k]][0] for k in range(5)] for e in top_results_and_scores]
                
                # remove duplicated paper
                if hop_2_papers_temp[0][0] == hop_1_paper:
                    hop_2_paper = hop_2_papers_temp[0][1]
                else:
                    hop_2_paper = hop_2_papers_temp[0][0]
                hop_2_papers.append(hop_2_paper)
            
            papers = [f"<p1>{hop_1_paper}</p1><p2>{hop_2_paper}</p2>" for hop_1_paper, hop_2_paper in zip(hop_1_papers, hop_2_papers)]
            
            # generate output
            output = generate(query_org, papers, keywords)
        ...
        out_file_path = f"/home/stv10121/RUS/dpr_result/keyword_mesh_{mode}.json"
